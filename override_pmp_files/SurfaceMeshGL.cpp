...
void SurfaceMeshGL::deleteBuffers()
{
    ...
    if(color_buffer_ != 0)          glDeleteBuffers(1, &color_buffer_);
    if(normal_buffer_ != 0)         glDeleteBuffers(1, &normal_buffer_);
    if(tex_coord_buffer_ != 0)      glDeleteBuffers(1, &tex_coord_buffer_);
    if(edge_buffer_ != 0)           glDeleteBuffers(1, &edge_buffer_);
    if(feature_buffer_ != 0)        glDeleteBuffers(1, &feature_buffer_);
    if(vertex_array_object_ != 0)   glDeleteVertexArrays(1, &vertex_array_object_);
